...
# --- ВРЕМЕННЫЙ ХЕЛПЕР ДЛЯ ПОЛУЧЕНИЯ FILE_ID ---
# Отправь боту картинку баннера в чат, чтобы получить её внутренний Telegram ID
@router.message(F.photo)
async def get_photo_id(message: Message):
    await message.answer(f"ID твоей картинки для главного меню:\n<code>{message.photo[-1].file_id}</code>\n\nСкопируй его и вставь в переменную MAIN_MENU_PHOTO.")
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    text = (
        ...
        "Выберите нужное вам action:"
    )
    try:
        await message.answer_photo(photo=MAIN_MENU_PHOTO, caption=text, reply_markup=main_menu_kb())
    except Exception:
...
async def back_to_main(call: CallbackQuery, state: FSMContext):
    ...
    try:
        await call.message.answer_photo(photo=MAIN_MENU_PHOTO, caption=text, reply_markup=main_menu_kb())
    except Exception:
...
# --- КНОПКИ: ИНФОРМАЦИЯ О САЛОНЕ ---
@router.callback_query(F.data == "menu_price")
async def show_price(call: CallbackQuery):
    text = (
        "💵 <b>Прайс-лист:</b>\n\n"
        "💅<b>Маникюр и покрытие</b>\n"
        "Маникюр (без покрытия) - 1500₽\n"
        "Маникюр + Гель-лак (однотонное) - 2500₽\n"
        "Японский маникюр (лечебный) - 2200₽\n"
        "Мужской маникюр - 1800₽\n"
        "Детский маникюр - 900₽\n\n"
        "👩‍🎨<b>​Дизайн</b>\n"
        "Френч - 2000₽\n"
        "Градиент (все ногти) - 3200₽\n"
        "Дизайн 1 ногтя (стемпинг/слайдер) - 150₽\n"
        "Художественная роспись 1 ногтя - 250₽\n"
        "Инкрустация стразами (1 ноготь) - 200₽\n\n"
        "🛠<b>​Укрепление и ремонт</b>\n"
        "Укрепление ногтей (гель/акрил) - 800₽\n"
        "Наращивание ногтей (полное) - 4500₽\n"
        "Коррекция наращивания - 3500₽\n"
        "Ремонт/достраивание 1 ногтя - 300₽\n\n"
        "🦶<b>​Педикюр</b>\n"
        "Педикюр (без покрытия) - 2500₽\n"
        "Педикюр + Гель-лак - 3700₽\n"
        "Обработка только пальчиков - 1800₽\n\n"
        "🛁<b>​Уход и снятие</b>\n"
        "Снятие чужого покрытия - 300₽\n"
        "Снятие без последующего покрытия - 500₽\n"
...
async def show_contacts(call: CallbackQuery):
    text = (
        ...
        "<i>Убедительная просьба: пожалуйста, обращайтесь по указанным контактам только по важным вопросам, касающимся вашей записи или услуг. Благодарим за понимание и уважение к личному времени наших сотрудников! 🌸</i>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_main")]])
    try:
        await call.message.delete()
...
async def show_portfolio(call: CallbackQuery):
    text = (
        "💄 Наше портфолио вы можете посмотреть в наших соцсетях:\n"
        "VK: ссылка_на_вк\n"
...
async def confirm_booking(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "confirm_no":
        ...
        await call.message.answer_photo(photo=MAIN_MENU_PHOTO, caption=menu_text, reply_markup=main_menu_kb())
        await state.clear()
        return
    ...
    # Уведомление админов с кнопкой отмены
    admin_text = f"🔔 <b>Новая запись! (Id{booking_id})</b>\nМастер: {data['master']}\nДата: {data['date']} {data['time']}\nКонтакт: {data['contact_info']}"
    
    cancel_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отменить с причиной", callback_data=f"adm_notify_can_{booking_id}")]
    ])
    
    for admin_id in ADMINS:
        try: await bot.send_message(admin_id, admin_text, reply_markup=cancel_kb)
        except: pass
    await state.clear()
...
# --- ОТМЕНА ЗАПИСИ ИЗ УВЕДОМЛЕНИЯ С ПРИЧИНОЙ ---
@router.callback_query(F.data.startswith("adm_notify_can_"))
async def admin_notify_cancel_start(call: CallbackQuery, state: FSMContext):
    if call.from_user.id not in ADMINS: return
    
    b_id = int(call.data.split("_")[3])
    
    await state.update_data(cancel_booking_id=b_id)
    await state.set_state(AdminState.entering_cancel_reason)
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Оставить запись (Отмена)", callback_data="admin_cancel_abort")]
    ])
    
    await call.message.edit_text(
        f"{call.message.html_text}\n\n✍️ <b>Отправьте следующим сообщением причину отмены для клиента:</b>", 
        reply_markup=kb
    )
    await call.answer()

@router.callback_query(F.data == "admin_cancel_abort", StateFilter(AdminState.entering_cancel_reason))
async def admin_cancel_abort(call: CallbackQuery, state: FSMContext):
    await state.clear()
    original_text = call.message.html_text.split("\n\n✍️")[0]
    await call.message.edit_text(original_text)
    await call.answer("Действие отменено. Запись сохранена ✅")

@router.message(AdminState.entering_cancel_reason)
async def process_cancel_reason(message: Message, state: FSMContext):
    if message.from_user.id not in ADMINS: return
    
    data = await state.get_data()
    b_id = data.get("cancel_booking_id")
    reason = message.text
    ...
    async with aiosqlite.connect('salon.db') as db:
        ...
        if not row:
            await message.answer("Эта запись уже была удалена ранее.")
            await state.clear()
            return
            
        user_id, master, date, time_val = row
        await db.execute("DELETE FROM bookings WHERE id=?", (b_id,))
        await db.commit()
        
    client_msg = (
        f"❗ <b>Отмена записи</b>\n"
        f"Ваша запись к мастеру <b>{master}</b> на <b>{date}</b> в <b>{time_val}</b> была отменена администратором.\n\n"
        f"<b>Причина отмены:</b>\n{reason}"
    )
    
    try:
        await bot.send_message(user_id, client_msg)
        client_notified = "Уведомление клиенту отправлено ✅"
    except Exception:
        client_notified = "Не удалось отправить уведомление (возможно, клиент заблокировал бота) ❌"
        
    await message.answer(f"🗑 Запись Id{b_id} успешно удалена.\n{client_notified}")
    await state.clear()

# --- 1. ПРОСМОТР ЗАПИСЕЙ И ОТМЕНА ИЗ МЕНЮ ---
